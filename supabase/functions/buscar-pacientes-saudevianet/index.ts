...
const handler = async (req: Request): Promise<Response> => {
  ...
  // Apenas admin autenticado (esta função é chamada pelo painel admin).
  const authHeader = req.headers.get("Authorization");
  if (!authHeader) {
    return new Response(
      JSON.stringify({ sucesso: false, erro: "Unauthorized", total_pacientes: 0, pacientes: [] }),
  ...
  }
  {
    ...
    if (!roleData) {
      return new Response(
        JSON.stringify({ sucesso: false, erro: "Forbidden", total_pacientes: 0, pacientes: [] }),
