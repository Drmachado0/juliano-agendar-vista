...
serve(async (req) => {
  ...
  try {
    ...
    // O provedor aceita URL pública OU data URI base64. Priorizamos URL.
    ...
    console.log('[enviar-whatsapp-imagem] Enviando via n8n (', usedMethod, ')');
    ...
    if (!result.success) {
      ...
      await registrarMensagemWhatsapp(supabase, {
        ...
        status_envio: "erro",
        error_message: `[WhatsApp] ${result.errorMessage ?? 'Erro desconhecido'}`,
      });
      return new Response(
