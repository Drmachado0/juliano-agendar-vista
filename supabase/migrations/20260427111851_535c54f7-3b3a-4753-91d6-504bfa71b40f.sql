-- RPC universal para registrar qualquer mensagem WhatsApp (IN ou OUT)
-- - Normaliza telefone
-- - Busca agendamento_id pelo último 8 dígitos quando não fornecido
-- - Categoriza tipo_mensagem
-- - Aceita status_envio='erro' + error_message para auditoria completa
...
DECLARE
  ...
  v_tipos_validos text[] := ARRAY[
    'manual','confirmacao','lembrete_24h','boas_vindas',
...
BEGIN
  ...
  -- Normaliza categoria (aceita não listadas, mas grava como sistema se inválida)
  IF NOT (v_tipo = ANY(v_tipos_validos)) THEN
    v_tipo := 'sistema';
  END IF;

  -- Vincular agendamento_id quando não veio
...
-- Permitir chamada via service_role (edge functions) e admin (UI)
